	...
	builds := []manifest.BuildInput{
		{
			...
			Path:     filepath.Join(*artifactRoot, "linux", "x86_64", "blazium-cli"),
			BaseURL:  fmt.Sprintf("https://cdn.blazium.app/cli/linux/x86_64/%s", *version),
			SigURL:   fmt.Sprintf("https://cdn.blazium.app/cli/linux/x86_64/%s/blazium-cli.sig", *version),
			Signing:  "gpg",
		},
		{
			Platform: "linux",
			Arch:     "x86_32",
			Filename: "blazium-cli",
			Path:     filepath.Join(*artifactRoot, "linux", "x86_32", "blazium-cli"),
			BaseURL:  fmt.Sprintf("https://cdn.blazium.app/cli/linux/x86_32/%s", *version),
			SigURL:   fmt.Sprintf("https://cdn.blazium.app/cli/linux/x86_32/%s/blazium-cli.sig", *version),
			Signing:  "gpg",
		},
		{
			Platform: "windows",
			...
			Path:     filepath.Join(*artifactRoot, "windows", "x86_64", "blazium-cli.exe"),
			BaseURL:  fmt.Sprintf("https://cdn.blazium.app/cli/windows/x86_64/%s", *version),
			Signing:  "sslcom",
		},
		{
			Platform: "windows",
			Arch:     "x86_32",
			Filename: "blazium-cli.exe",
			Path:     filepath.Join(*artifactRoot, "windows", "x86_32", "blazium-cli.exe"),
