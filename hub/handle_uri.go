...
func RunInstall(p InstallParams) (map[string]any, error) {
	...
	if strings.TrimSpace(p.Channel) != "" {
		resolvedChannel, err = NormalizeChannel(p.Channel)
		if err != nil {
			return nil, err
		}
	}
	...
	if p.WithTemplates {
		...
		installed, err := exporttemplates.InstallTPZ(baseVersion, p.Mono, resolvedChannel == ChannelNightly, tplDest)
		if err != nil {
...
func HandleURI(opts HandleURIOptions) (map[string]any, error) {
	...
	case "hub":
		out, err := FocusHub(60 * time.Second)
		if err != nil {
			return nil, err
		}
		out["uri"] = parsed.Raw
	...
	case "register":
		return handleURIRegister(parsed)
...
func handleURIRegister(parsed ParsedURI) (map[string]any, error) {
	...
	ed, err := f.AddEditorFromPath(resolved, parsed.Version, platform, arch, parsed.Channel, parsed.Mono)
	if err != nil {
		return nil, err
	}
	if err := Save(f); err != nil {
